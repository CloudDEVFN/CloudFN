...
express.get("/launcher/api/public/distributionpoints/", async (req, res) => {
    res.json({
        "distributions": [
            ...
            "https://cloudfn.ol.epicgames.com/"
        ]
    });
})
...
express.get("/launcher/api/public/assets/*", async (req, res) => {
    res.json({
        ...
        "items": {
            "MANIFEST": {
                "signature": "CloudFN",
                "distribution": "https://cloudfn.ol.epicgames.com/",
                "path": "Builds/Fortnite/Content/CloudDir/CloudFN.manifest",
...
express.get("/Builds/Fortnite/Content/CloudDir/*.manifest", async (req, res) => {
    ...
    const manifest = fs.readFileSync(path.join(__dirname, "..", "responses", "CloudDir", "CloudFN.manifest"));
...
express.get("/Builds/Fortnite/Content/CloudDir/*.chunk", async (req, res) => {
    ...
    const chunk = fs.readFileSync(path.join(__dirname, "..", "responses", "CloudDir", "CloudFN.chunk"));
...
express.post("/fortnite/api/game/v2/chat/*/*/*/pc", async (req, res) => {
    res.json({ "GlobalChatRooms": [{"roomName":"cloudfnglobal"}] })
})
