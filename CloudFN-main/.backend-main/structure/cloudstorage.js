...
express.get("/fortnite/api/cloudstorage/user/*/:file", async (req, res) => {
    try {
        if (!fs.existsSync(path.join(process.env.LOCALAPPDATA, "CloudFN", "ClientSettings"))) {
            fs.mkdirSync(path.join(process.env.LOCALAPPDATA, "CloudFN", "ClientSettings"));
        }
    } catch (err) {}
    ...
    let file;
    if (process.env.LOCALAPPDATA) file = path.join(process.env.LOCALAPPDATA, "CloudFN", "ClientSettings", `ClientSettings.Sav`);
    else file = path.join(__dirname, "..", "ClientSettings", `ClientSettings.Sav`);
